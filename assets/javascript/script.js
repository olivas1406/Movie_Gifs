...
$(document).ready(function(){ 
    ...
    $("button").on("click", function() {                                    // listen for click event
        $(".aGreatMan").empty();                                            // Clear the previous Gifs
        var greatMan = $(this).attr("data-name");                           // Pull the name of the button from the data-name attribute
        var queryURL = "http://api.giphy.com/v1/gifs/search?q=" + greatMan +"&api_key=9OITCt8vj9Ey0OaPr8PoA9Qo3JAL9gxx&limit=10"; // giphy API URL
        ...
        $.ajax({                                                            // AJAX request
            ...
            var results = response.data;                                            // Create variable 'results' to hold resonse.data  
                for (var  j= 0; j < results.length; j++) {                          // For Loop to loop through the 10 results
                    var gifDiv = $("<div class='aGreatMan'>");                      // create a div class called 'aGreatMan'
                    ...
                    var personImage = $("<img>");                                   // Create an IMG HTML element
                    personImage.attr("src", results[j].images.fixed_height.url);    // Give the IMG a source from the results
                    gifDiv.prepend(p);                                              // Add the rating to the gif's div
                    gifDiv.prepend(personImage);                                    // Add the IMG element and source to the gif's div
